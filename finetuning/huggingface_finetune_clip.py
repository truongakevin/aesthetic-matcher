...
def main():
    ...
    if training_args.do_train:
        ...
        if training_args.resume_from_checkpoint is not None:
            print("------resuming from checkpoint arg", training_args.resume_from_checkpoint)
            checkpoint = training_args.resume_from_checkpoint
        elif last_checkpoint is not None:
            print("------resuming from checkpoint la", last_checkpoint)
            if not os.path.exists(f"{last_checkpoint}/trainer_state.json"):
                checkpoint_number = int(last_checkpoint.split('-')[-1])
                new_checkpoint_number = checkpoint_number - 500
                last_checkpoint = f"{'-'.join(last_checkpoint.split('-')[:-1])}-{new_checkpoint_number}"
                old_checkpoint = f"{'-'.join(last_checkpoint.split('-')[:-1])}-{checkpoint_number}"
